...
class ZapretGUI(ctk.CTk):
    ...
    def _apply_path(self) -> None:
        ...
        has_service = bool(self._find_in_tree(path, SERVICE_BAT, max_depth=1))
        has_winws   = bool(self._find_in_tree(path, WINWS_EXE))

        # zapret в папке не обнаружен — предлагаем скачать и установить
        if not has_service and not has_winws:
            if messagebox.askyesno(
                "zapret не найден",
                f"В указанной папке не найден zapret "
                f"(нет {SERVICE_BAT} и {WINWS_EXE}).\n\n"
                "Скачать и установить последнюю версию zapret в эту папку?",
            ):
                self._install_zapret(path)
                return
            # пользователь отказался — даём задать путь вручную (поведение как раньше)
    ...
    def _commit_path(self, path: str) -> None:
        """Сохраняет путь к zapret и обновляет все зависящие от него
        части интерфейса (стратегии, конфиги, фильтры)."""
    ...
    def _install_zapret(self, path: str) -> None:
        """Скачивает последний релиз zapret и распаковывает его в папку path.
        Использует тот же механизм, что и обновление (ZapretUpdater)."""
        if not os.access(path, os.W_OK):
            messagebox.showerror("Ошибка", f"Нет прав на запись в папку:\n{path}")
            return

        self._set_update_status("Получение ссылки на загрузку…", ACCENT)
        self._upd_check_btn.configure(state="disabled")
        self._upd_apply_btn.configure(state="disabled")

        def work():
            # 1. ищем прямую ссылку на .zip последнего релиза
            try:
                url = UpdateChecker(path).fetch_download_url()
            except Exception as e:  # noqa: BLE001
                self.after(0, self._finish_install, path, False,
                           "Не удалось получить ссылку на загрузку: "
                           + UpdateChecker.friendly_error(e))
                return
            if not url:
                self.after(0, self._finish_install, path, False,
                           "Не удалось найти архив релиза zapret на GitHub.")
                return
            # 2. скачиваем и распаковываем (на пустой папке — просто установка)
            ok, msg = ZapretUpdater(path, url).run(
                progress=lambda m: self.after(0, self._set_update_status, m, ACCENT)
            )
            self.after(0, self._finish_install, path, ok, msg)

        threading.Thread(target=work, daemon=True).start()

    def _finish_install(self, path: str, ok: bool, msg: str) -> None:
        self._upd_check_btn.configure(state="normal")
        if ok:
            self._path_var.set(path)
            self._commit_path(path)
            ver = read_local_zapret_version(path) or "?"
            self._set_update_status(f"✔  zapret установлен (версия {ver}).", GREEN)
            log.info(f"zapret установлен в {path}, версия {ver}")
            messagebox.showinfo(
                "Готово", f"zapret успешно установлен (версия {ver}) в:\n{path}")
        else:
            self._set_update_status("✖  " + msg, RED)
            log.error(f"Установка zapret не удалась: {msg}")
            messagebox.showerror("Ошибка установки", msg)
